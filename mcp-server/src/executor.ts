...
const processTracker: ProcessTracker = {
  register(executionId: string, child: ChildProcess) {
    if (!activeProcesses.has(executionId)) activeProcesses.set(executionId, new Set());
    activeProcesses.get(executionId)!.add(child);
  },
  unregister(executionId: string, child: ChildProcess) {
    activeProcesses.get(executionId)?.delete(child);
  },
};

/** Wrapper for runClaude that injects processTracker. */
function runClaudeTracked(
  prompt: string,
  step: ChainStep,
  ...
  return runClaude(prompt, step, onChunk, executionId, timeoutMs, processTracker);
}

// ─── Pre-tool executor (delegated to pretool-executor.ts) ──────────────────
// See pretool-executor.ts, claude-runner.ts, gate-manager.ts
...
async function executeStep(
  ...
  if (stepType === "gate") {
    ...
    const existingApproval = getGateResult(`${executionId}:${stepId}`);
    if (existingApproval) {
      deleteGateResult(`${executionId}:${stepId}`);
    ...
    const gateTimer = setTimeout(() => {
      if (!hasGateResult(gateKey)) {
        // Auto-resolve on timeout
        if (onTimeout === "approve") {
          setGateResult(gateKey, "approved");
        } else if (onTimeout === "skip") {
          setGateResult(gateKey, "skipped");
        } else {
          setGateResult(gateKey, "rejected");
        }
      }
    }, timeoutMs);
    setGateTimer(gateKey, gateTimer);
